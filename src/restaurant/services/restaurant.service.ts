...
export class RestaurantService {
  ...
  async create(restaurant: CreateRestaurantDto) {
    if (!restaurant.isAdmin) {
      const isSirenValid = await this.sirenDataGateway.checkSiren(
    ...
    const newRestaurant =
      this.restaurantDataGateway.createRestaurant(restaurant);

    // check if the password length is greater than 7
    if (newRestaurant.password.length < 7) {
      throw new HttpException(
        'the minimum of characters is 7 for your password',
        HttpStatus.UNPROCESSABLE_ENTITY,
      );
    }

    // hash the password
    newRestaurant.password = await this.bcrypt.hashSync(
      newRestaurant.password,
      this.saltOrRounds,
    );

    // save the restaurant in the database
    const result = await this.restaurantDataGateway.saveRestaurant(
      newRestaurant,
    );
  ...
  async findAll() {
    const restaurants = await this.restaurantDataGateway.getAllRestaurants();
    if (!restaurants) {
  ...
  async findOneByEmail(email: string) {
    const restaurant = await this.restaurantDataGateway.getRestaurantByEmail(
      email,
    );
    if (!restaurant) {
  ...
  async update(
    ...
    const restaurant_update =
      await this.restaurantDataGateway.getRestaurantByEmail(email);
    if (!restaurant_update) {
      throw new NotFoundException('Restaurant not found');
    }
    const result = await this.restaurantDataGateway.updateRestaurant(
      email,
      restaurant_update,
    );
    this.hidePassword(result);
  ...
  async remove(companyName: string) {
    const restaurant =
      await this.restaurantDataGateway.getRestaurantByCompanyName(companyName);
    if (!restaurant) {
      throw new NotFoundException('Restaurant not found');
    }
    const result = await this.restaurantDataGateway.deleteRestaurantByEmail(
      restaurant.email,
    );
  ...
  async createFirstRestaurant() {
    const password = await this.bcrypt.hashSync(
      process.env.ROOT_USER_PASSWORD || 'string',
      this.saltOrRounds,
    );
    const res = await this.restaurantDataGateway
